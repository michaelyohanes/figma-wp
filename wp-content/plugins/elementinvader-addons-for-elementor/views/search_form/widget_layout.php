...
<div class="widget-eli eli-blog-search contact-form" id="eli_<?php echo esc_html($this->get_id_int());?>">
    <div class="eli-container">
        <form class="eli_f elementinvader_addons_for_elementor_f" id="searchform" method="get" action="<?php echo esc_attr((!empty($settings['special_result_page'])) ? $settings['special_result_page'] : bloginfo('url')); ?>">
            <?php
                if(!empty($settings['special_result_page'])) {
                    $query_params = [];
                    $parsed_url = wp_parse_url($settings['special_result_page']);
            ...
            <div class="eli_f_container elementinvader_addons_for_elementor_f_container">
                <div class="eli_f_group eli_f_group_el_s ">
                    ...
                    <?php
                        wp_nonce_field('eli_blog_search_form_action', 'eli_blog_search_form_nonce');
                    ?>
   
                    <input type="text" class="eli_f_field" name="<?php echo esc_attr((!empty($settings['special_result_page'])) ? 'search' : 's'); ?>" id="s" placeholder="<?php echo esc_attr($settings['field_search_placeholder_text']); ?>" value="<?php echo esc_attr((isset($_GET['search'])) ? esc_attr(sanitize_text_field(wp_unslash($_GET['search']))) : esc_attr(get_search_query())); // phpcs:ignore WordPress.Security.NonceVerification.Recommended -- Read-only search parameter.?>" />
                </div>
